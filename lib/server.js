...
var require_auth = __commonJS({
  "src/auth.js"(exports2, module2) {
    ...
    function checkAuth2(req, authConfig) {
      ...
      const looksLikeServeProxy = mode === "tailscale" && tsCfg.verifyServeOrigin === true && typeof req.headers[AUTH_HEADERS.tailscale.login] === "string" && req.headers[AUTH_HEADERS.tailscale.login].length > 0;
...
var require_dispatch = __commonJS({
  "src/dispatch.js"(exports2, module2) {
    ...
    function createDispatch2(options = {}) {
      ...
      function startRun(node, { args: args2, agent, message, sessionKey }) {
        ...
        return Promise.resolve(resolveAgentNode(agentRef)).then((route) => {
      ...
      function dispatchTask(taskId, opts = {}) {
        ...
        let settledPromise;
        try {
          settledPromise = startRun(opts.node, { args: args2, agent, message, sessionKey });
        } catch (e) {
  ...
  "src/fleet-routes.js"(exports2, module2) {
    ...
    function createFleetRoutes2({
      ...
      async function handleOrgChart(req, res, method, segments) {
        if (!orgChart2 || segments.length !== 1) return false;
        if (method === "GET") {
          json(res, 200, orgChart2.getChart());
          return true;
        }
        if (method === "PUT") {
          const user = guardMutation(req, res);
          if (!user) return true;
          const body = await readJsonBody(req);
          const chart = orgChart2.replaceChart({ roots: body.roots, unassigned: body.unassigned }, user);
          recordAudit2(user, "org.update", null, {
...
var require_orchestrate = __commonJS({
  "src/orchestrate.js"(exports2, module2) {
    ...
    var RESULT_TEXT_NOTE_PREFIX = "result: ";
      ...
      function runBoard(params = {}) {
        ...
        const runner = async () => {
          ...
          const seats = agents.map((agent, i) => {
            ...
            try {
              const dispatched = dispatch2.dispatchTask(card.id, { agent, actor, isBoard: true });
      ...
      function runChain(params = {}) {
        ...
        const runner = async () => {
          ...
          for (let i = 0; i < steps.length; i++) {
            ...
            let dispatched;
            try {
              dispatched = dispatch2.dispatchTask(card.id, { agent: step.agent, actor });
            } catch (e) {
              results.push({
                agent: step.agent,
                taskId: card.id,
...
// src/org-chart.js
var require_org_chart = __commonJS({
  "src/org-chart.js"(exports2, module2) {
    var path2 = require("path");
    var { createSafeStore } = require_state_safety();
    var CHART_VERSION = 1;
    var NODE_FIELDS = Object.freeze(["agentId", "title", "children"]);
    ...
    function validateChart(obj) {
      ...
      const seen = { ids: /* @__PURE__ */ new Set(), count: 0, overflow: false };
    ...
    function normalizeNode(node) {
      ...
      return {
        ...
        title: node.title === void 0 || node.title === "" ? null : node.title,
    ...
    function normalizeChart(input = {}) {
      ...
      const chart = {
        version: input.version === void 0 ? CHART_VERSION : input.version,
        updated_at: (/* @__PURE__ */ new Date()).toISOString(),
    ...
    function createOrgChart2(options = {}) {
      const { stateDir, onChange, debounceMs } = options;
      if (!stateDir) throw new Error("createOrgChart: stateDir is required");
      const store = createSafeStore({
        filePath: path2.join(stateDir, "org-chart.json"),
        backupDir: path2.join(stateDir, ".backups"),
      ...
      function emit(type, detail) {
        if (typeof onChange === "function") {
          onChange({ type, ts: (/* @__PURE__ */ new Date()).toISOString(), ...detail });
        }
      }
      function getChart() {
        return store.read().data;
    ...
    module2.exports = {
      ...
      normalizeChart,
      countNodes,
      createOrgChart: createOrgChart2
    };
  }
});
...
var { createOrgChart } = require_org_chart();
