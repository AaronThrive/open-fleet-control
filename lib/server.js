  ...
  "src/dispatch.js"(exports2, module2) {
    ...
    function composeKickoffMessage(task, { agent, baseUrl, briefsDir, slackChannel, isBoard, slack = {}, slackThreadTs = null }) {
      ...
      const lines = [
        ...
        ...isBoard ? [`   Because this is a BOARD task, lead the post with "${chiefMention}" and light emojis are welcome.`] : [`   Keep it factual and self-contained \u2014 a teammate reading only the Slack post should understand the outcome.`]
      ];
      return lines.join("\n");
    }
    ...
    function createDispatch2(options = {}) {
      ...
      function dispatchTask(taskId, opts = {}) {
        ...
        const completion = settledPromise.then(
          (settled) => handleRunSettled(taskId, attemptIndex, agent, settled, startedMs, isBoard ? "done" : "review")
        );
...
var { runOpenClaw, runOpenClawAsync, runOpenClawArgv, extractJSON, getSafeEnv } = require_openclaw();
var {
  getSystemVitals,
  forceRefreshVitals,
  getVitalsCacheAgeMs,
