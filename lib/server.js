...
var require_utils = __commonJS({
  "src/utils.js"(exports2, module2) {
    var { exec, execFile: execFile2 } = require("child_process");
    ...
    async function runCmdSafe(file, args2 = [], options = {}) {
      const systemPath = "/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin";
      const envPath = process.env.PATH || "";
      const opts = {
        encoding: "utf8",
      ...
      try {
        const { stdout } = await execFileAsync(file, args2, opts);
        return stdout.trim();
      } catch (e) {
        if (options.fallback !== void 0) return options.fallback;
        throw e;
  ...
  "src/config.js"(exports2, module2) {
    ...
    var FLEET_DEFAULTS = {
      ...
      alerts: {
        ...
        ingest: {
          ntfy: { enabled: false, intervalMs: 3e4 },
          cron: { enabled: false, intervalMs: 12e4 }
        }
      },
...
var require_openclaw = __commonJS({
  "src/openclaw.js"(exports2, module2) {
    ...
    async function runOpenClawArgv2(argv, { timeout = 2e4 } = {}) {
      const profile = process.env.OPENCLAW_PROFILE || "";
    ...
    module2.exports = {
      ...
      runOpenClawArgv: runOpenClawArgv2,
  ...
  "src/cron.js"(exports2, module2) {
    ...
    function mapOpenClawJob(job, node) {
      ...
      return {
        ...
        nextRunAtMs: Number.isFinite(state2.nextRunAtMs) ? state2.nextRunAtMs : null,
    ...
    var HOST_CRONTAB_TIMEOUT_MS = 5e3;
    function cronFieldValues(field, min, max) {
      ...
      for (const part of String(field).split(",")) {
        ...
        if (range !== "*") {
          ...
          lo = parseInt(a, 10);
          hi = b !== void 0 ? parseInt(b, 10) : a !== void 0 ? parseInt(a, 10) : NaN;
    ...
    function cronNextRunMs(expr, fromMs) {
      ...
      const dows = cronFieldValues(parts[4], 0, 7);
      ...
      const horizon = fromMs + 366 * 24 * 60 * 60 * 1e3;
      while (d.getTime() <= horizon) {
        const dow = d.getDay();
        const dowMatch = dows.has(dow) || dow === 0 && dows.has(7);
        const domMatch = doms.has(d.getDate());
        const dayMatch = domRestricted && dowRestricted ? domMatch || dowMatch : (!domRestricted || domMatch) && (!dowRestricted || dowMatch);
        if (months.has(d.getMonth() + 1) && dayMatch && hours.has(d.getHours()) && minutes.has(d.getMinutes())) {
          return d.getTime();
        }
        d.setMinutes(d.getMinutes() + 1);
      }
    ...
    function hostCronLastRunMs(command) {
      ...
      if (target.startsWith("~/")) target = path2.join(os2.homedir(), target.slice(2));
      if (!path2.isAbsolute(target)) return null;
      const resolved = path2.resolve(target);
      if (!resolved.startsWith(os2.homedir() + path2.sep)) return null;
      try {
        const st = fs2.statSync(resolved);
        return st.isFile() ? Math.round(st.mtimeMs) : null;
      } catch {
        return null;
      }
    }
    var HOST_JOURNAL_SINCE = "7 days ago";
    var HOST_JOURNAL_TIMEOUT_MS = 5e3;
    var HOST_JOURNAL_MAX_BUFFER = 16 * 1024 * 1024;
    var CRON_JOURNAL_LINE_RE = /^(\w{3}\s+\d{1,2}\s+\d{2}:\d{2}:\d{2})\s.*\bCMD\s*\((.*)\)\s*$/;
    var MONTHS = {
      Jan: 0,
      Feb: 1,
      Mar: 2,
    ...
    function parseCronJournalTs(stamp, nowMs) {
      ...
      if (candidate > nowMs + 24 * 60 * 60 * 1e3) {
    ...
    var cronJournalReader = defaultCronJournalReader;
    ...
    var hostCrontabReader = defaultHostCrontabReader;
    function getHostCronJobs(node) {
      ...
      return String(raw).split("\n").map((line, i) => mapHostCronLine(line, i, node, journal)).filter((job) => job !== null);
    }
    ...
    function getCronJobs2(getOpenClawDir2, opts = {}) {
      ...
      let openclawJobs = [];
      try {
        openclawJobs = getOpenClawRawJobs(getOpenClawDir2).map((j) => mapOpenClawJob(j, node)).map(overlayLatestRun);
      } catch (e) {
...
var require_tokens = __commonJS({
  "src/tokens.js"(exports2, module2) {
    ...
    var PROVIDER_TOKEN_RATES = {
      // Anthropic Claude (Opus 4) — same as the default table.
      "claude-code": { input: 15, output: 75, cacheRead: 1.5, cacheWrite: 18.75 },
    ...
    var BILLING_SUBSCRIPTION = "subscription";
    var BILLING_PER_TOKEN = "per-token";
    var DEFAULT_BILLING_MODE = BILLING_PER_TOKEN;
    var DEFAULT_CLAUDE_PLAN_COST = 200;
    var DEFAULT_CLAUDE_PLAN_NAME = "Claude Code Max";
    var DEFAULT_CODEX_PLAN_COST = 200;
    var DEFAULT_CODEX_PLAN_NAME = "Codex subscription";
    function getBillingPlans(config) {
      const billing = config && config.billing || {};
      if (Array.isArray(billing.plans)) {
        const plans = billing.plans.filter((p) => p && typeof p === "object").map((p) => ({
          ...
          providers: Array.isArray(p.providers) ? p.providers.map((x) => String(x)).filter(Boolean) : []
        }));
        if (plans.length > 0) return plans;
      }
      const claudeCost = Number.isFinite(Number(billing.claudePlanCost)) ? Number(billing.claudePlanCost) : DEFAULT_CLAUDE_PLAN_COST;
      const codexCost = Number.isFinite(Number(billing.codexPlanCost)) ? Number(billing.codexPlanCost) : DEFAULT_CODEX_PLAN_COST;
      return [
        {
          id: "claude",
    ...
    function resolveBillingMode(provider, providerBillingModes) {
      const map = providerBillingModes && typeof providerBillingModes === "object" ? providerBillingModes : {};
    ...
    var CLAUDE_PROJECTS_SCAN_DEPTH = 4;
    ...
    async function refreshTokenUsageAsync2(getOpenClawDir2, getClaudeProjectsDir) {
      ...
      try {
        ...
        const claudeProjectsDir = typeof getClaudeProjectsDir === "function" ? getClaudeProjectsDir() : path2.join(require("os").homedir(), ".claude", "projects");
    ...
    function aggregatePlanCosts(byModel = {}, plans = [], rates = TOKEN_RATES, providerBillingModes = {}) {
      const providerToPlan = /* @__PURE__ */ new Map();
    ...
    function summarizeModelUsage(byModel = {}, rates = TOKEN_RATES, providerBillingModes = {}) {
      return Object.entries(byModel).map(([model, bucket]) => {
        ...
        return {
          ...
          marginalCost,
          // real $ — 0 for subscription
          hypotheticalCost,
          // "if billed per-token" projection
    ...
    function getCostBreakdown2(config, getSessions, getOpenClawDir2) {
      ...
      for (const [key, windowConfig] of Object.entries(windowConfigs)) {
        ...
        const split = bucket.byModel ? aggregateModelCosts(bucket.byModel, TOKEN_RATES, providerBillingModes) : (() => {
        ...
        const planLines = aggregatePlanCosts(
          ...
          const hypotheticalMonthly2 = line.hypotheticalCost / windowConfig.days * 30;
          const marginalMonthly2 = line.marginalCost / windowConfig.days * 30;
          const actualMonthlySpend2 = line.planCost + marginalMonthly2;
          const savings = hypotheticalMonthly2 - actualMonthlySpend2;
          return {
            ...line,
            ...
            hypotheticalMonthly: hypotheticalMonthly2,
            marginalMonthly: marginalMonthly2,
            actualMonthlySpend: actualMonthlySpend2,
        ...
        windows[key] = {
          ...
          actualMonthlySpend,
          monthlySavings,
          savingsPercent: monthlySavings > 0 && hypotheticalMonthly > 0 ? Math.round(monthlySavings / hypotheticalMonthly * 100) : 0,
    ...
    function getTokenStats2(sessions2, capacity, config = {}) {
      ...
      const providerBillingModes = getProviderBillingModes(config);
      const split24h = usage?.byModel ? aggregateModelCosts(usage.byModel, TOKEN_RATES, providerBillingModes) : (() => {
        const c = calculateCostForBucket(usage);
        return { marginalTotal: c.totalCost, hypotheticalTotal: c.totalCost, subscriptionTotal: 0 };
      })();
      const estCost = split24h.hypotheticalTotal;
      const marginalCost = split24h.marginalTotal;
      const plans = getBillingPlans(config);
      const planCost = totalPlanCost(plans);
      const planName = plans[0]?.label || DEFAULT_CLAUDE_PLAN_NAME;
      const monthlyApiCost = estCost * 30;
      const marginalMonthly = marginalCost * 30;
      const actualMonthlySpend = planCost + marginalMonthly;
      ...
      for (const [key, windowConfig] of Object.entries(windowConfigs)) {
        ...
        const wSplit = bucket.byModel ? aggregateModelCosts(bucket.byModel, TOKEN_RATES, providerBillingModes) : (() => {
          const c = calculateCostForBucket(bucket);
          return {
            marginalTotal: c.totalCost,
            hypotheticalTotal: c.totalCost,
        ...
        savingsWindows[key] = {
          ...
          savingsPercent: windowSavingsPositive && monthlyProjected > 0 ? Math.round(windowSavings / monthlyProjected * 100) : 0,
          requests: bucket.requests
        };
      }
      return {
        ...
        savingsPercent: savingsPositive && monthlyApiCost > 0 ? Math.round(monthlySavings / monthlyApiCost * 100) : 0,
...
var require_dispatch = __commonJS({
  "src/dispatch.js"(exports2, module2) {
    ...
    var SLACK_BOARD_FALLBACK = "channel:C0ANFEW6RU4";
    ...
    function composeKickoffMessage(task, { agent, baseUrl, briefsDir, slackChannel, isBoard, slack = {}, slackThreadTs = null }) {
      ...
      const replyToArg = isBoard && typeof slackThreadTs === "string" && slackThreadTs ? ` --reply-to ${slackThreadTs}` : "";
      if (isBoard) {
        return [
  ...
  "src/state.js"(exports2, module2) {
    ...
    function createStateModule2(deps) {
      ...
      function getSubagentStatus() {
        ...
        try {
          const cached = getSessions({ limit: null }) || [];
          const subagentSessions = cached.filter((s) => s && s.sessionType === "subagent").filter((s) => s.sessionType !== "main").map((s) => ({
            key: s.sessionKey || "",
            sessionId: s.sessionId,
          ...
          }));
          {
            for (const s of subagentSessions) {
              ...
              const transcript = Array.isArray(readTranscript(s.sessionId)) ? readTranscript(s.sessionId) : [];
              const userTexts = [];
  ...
  "src/tailscale.js"(exports2, module2) {
    ...
    function createTailscaleAdapter(deps = {}) {
      const {
        ...
        execFn = (file, args2 = []) => runCmdSafe(file, args2, { timeout: EXEC_TIMEOUT_MS }),
        fetchFn = (...args2) => globalThis.fetch(...args2),
  ...
  "src/mesh.js"(exports2, module2) {
    ...
    function groupNodesByHost(enrichedNodes) {
      ...
      for (const node of enrichedNodes) {
        const reportedHost = node.vitals && typeof node.vitals.hostname === "string" && node.vitals.hostname ? node.vitals.hostname : null;
...
${normalizeForDedup(body)}`;
      ...
      const prefix = normalizeForDedup(title).replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 60);
...
`);
    ...
    function createEvolution({ workspaceDir, stateDir, onChange, getGateDefault, lessonsVaultDir } = {}) {
      ...
      const vaultDir = typeof lessonsVaultDir === "string" && lessonsVaultDir.trim().length > 0 ? lessonsVaultDir.trim() : "";
...
`;
          const filePath = path2.join(vaultDir, `${vaultSlug(lesson.title, lesson.body)}.md`);
      ...
      function addLesson({ title, body, author } = {}) {
        ...
        const duplicate = sections.find(
          (s) => !s.parseError && normalizeForDedup(s.title) === incomingTitleKey && normalizeForDedup(s.body) === incomingBodyKey
        );
        if (duplicate) {
          const existing = {
            id: duplicate.id,
...
var require_cortex_gbrain = __commonJS({
  "src/cortex-gbrain.js"(exports2, module2) {
    ...
    var MIRROR_STALE_MS = 26 * 60 * 60 * 1e3;
    ...
    function parseStatsText(text) {
      ...
      if (Number.isFinite(chunks) && Number.isFinite(embedded) && chunks > 0) {
        embeddedCoverage = Math.round(embedded / chunks * 1e4) / 1e4;
      }
      return {
        pages,
        chunks,
    ...
    function readLogTail(filePath, readFileSyncFn) {
      ...
      const readSync = readFileSyncFn || ((p, o) => fs2.readFileSync(p, o));
      try {
    ...
    function normalizeGbrainDate(raw, slug) {
      ...
      const r = raw.trim();
      if (/\b\d{4}\b/.test(r)) return r;
      const ms = Date.parse(`${r} ${(/* @__PURE__ */ new Date()).getFullYear()}`);
    ...
    function createGbrain(options = {}) {
      ...
      const cliPath = options.cliPath || path2.join(home, "gbrain", "bin", "gbrain");
      const execFn = options.execFn || defaultExecFn;
      const vaultDir = options.vaultDir || path2.join(home, "OC Obsidian KB Vault");
      const syncLogPath = options.syncLogPath || path2.join(vaultDir, "_system", "logs", "gbrain-sync.log");
      const exportLogPath = options.exportLogPath || path2.join(home, "logs", "gbrain-export.log");
...
${res.stderr}`.trim().split("\n")[0]?.slice(0, 200) || "no output";
      ...
      async function list({ limit, offset = 0, query } = {}) {
        ...
        if (typeof query === "string" && query.trim() !== "") {
          ...
          items = items.filter(
            (item) => String(item.title).toLowerCase().includes(needle) || String(item.id).toLowerCase().includes(needle)
          );
        }
        const total = items.length;
        const start = Number.isFinite(offset) && offset > 0 ? Math.floor(offset) : 0;
        const sliced = Number.isFinite(limit) && limit >= 0 ? items.slice(start, start + Math.floor(limit)) : items.slice(start);
      ...
      async function stats() {
        ...
        if (!statsRes.error) {
          ...
          if (parsed) {
            ...
            embeddedCoverage = parsed.embeddedCoverage === null || Number.isFinite(parsed.embeddedCoverage) ? parsed.embeddedCoverage : null;
          }
        }
      ...
      async function healthStats() {
        ...
        const healthy = typeof s.embeddedCoverage === "number" ? s.embeddedCoverage >= 0.999 : null;
        return {
        ...
        try {
          ...
          if (ranged) {
            const inWindow = daily.filter(
              (day) => withinRange(parseDailyDate(day && day.date), from, to)
            );
          ...
          if (tables.includes("summaries")) {
            ...
            if (columns.includes("token_count")) {
              ...
              if (tables.includes("messages")) {
                try {
                  const msgCols = db.prepare("PRAGMA table_info(messages)").all().map((col) => col.name);
                  const msgRange = canRange && msgCols.includes("created_at");
                  const messages = db.prepare(`SELECT COUNT(*) AS n FROM messages${msgRange ? whereSql : ""}`).get(...msgRange ? whereParams : []);
  ...
  "src/cortex.js"(exports2, module2) {
    ...
    function createCortex(options = {}) {
      ...
      async function getStateRanged(range) {
        ...
        const hasRange = range && (range.from !== null && range.from !== void 0 && range.from !== "" || range.to !== null && range.to !== void 0 && range.to !== "");
      ...
      async function collectState() {
        ...
        try {
          ...
          state2.memory.available = !!gbrainAvailability.available;
          state2.memory.reason = gbrainAvailability.reason || null;
          if (gbrainAvailability.available) {
            const memoryStats = await gbrain.stats();
            if (memoryStats && !memoryStats.error) {
              state2.memory.pageCount = memoryStats.pageCount ?? null;
              state2.memory.lastUpdated = memoryStats.lastUpdated ?? null;
        ...
        try {
          ...
          state2.gbrain.available = !!availability.available;
          state2.gbrain.reason = availability.reason || null;
        } catch (e) {
        ...
        if (state2.gbrain.available && typeof gbrain.healthStats === "function") {
          try {
        ...
        if (state2.gbrain.available && typeof gbrain.obsidianHealth === "function") {
          try {
        ...
        if (state2.gbrain.available && typeof gbrain.recentUpdates === "function") {
          try {
            const r = await gbrain.recentUpdates(10);
            if (r && !r.error && Array.isArray(r.items)) {
              state2.recentUpdates = r.items.map((item) => ({
                id: item.id ?? null,
...
var require_alerts_history = __commonJS({
  "src/alerts-history.js"(exports2, module2) {
    ...
    function createAlertHistory({
      ...
      function clear() {
        try {
          if (!fs2.existsSync(activePath)) return { archived: false, backup: null };
          const stamp = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-");
          let backupPath = path2.join(logsDir, `alerts.${stamp}.cleared`);
          if (fs2.existsSync(backupPath)) {
            backupPath = path2.join(
              logsDir,
              `alerts.${stamp}-${crypto.randomBytes(3).toString("hex")}.cleared`
            );
          }
          fs2.renameSync(activePath, backupPath);
          return { archived: true, backup: path2.basename(backupPath) };
        } catch (e) {
...
var require_budgets = __commonJS({
  "src/budgets.js"(exports2, module2) {
    ...
    var DEFAULT_SUBSCRIPTION_PROVIDERS = ["claude-code"];
...
// src/subscription-limit-watcher.js
var require_subscription_limit_watcher = __commonJS({
  "src/subscription-limit-watcher.js"(exports2, module2) {
    var DEFAULT_WARN_PCT = 80;
    var DEFAULT_CRITICAL_PCT = 95;
    ...
    function normalizeConfig(raw) {
      ...
      const pollIntervalMs = Number.isFinite(pollRaw) && pollRaw >= MIN_POLL_INTERVAL_MS ? pollRaw : DEFAULT_POLL_INTERVAL_MS;
      return {
        enabled: src.enabled === true,
        warnPct,
        criticalPct,
      ...
      function emit(row, severity) {
        ...
        try {
          onAlert({
            ...
            message: `Subscription limit ${severity} (${row.provider} ${row.window}): ${pct}% of ${capText}` + (row.resetsAt ? ` \u2014 resets ${row.resetsAt}` : "")
          });
        } catch (e) {
...
// src/digests-store.js
var require_digests_store = __commonJS({
  "src/digests-store.js"(exports2, module2) {
    var crypto = require("crypto");
    var fs2 = require("fs");
    var path2 = require("path");
    var DIGEST_NAME_RE = /^[a-zA-Z0-9._-]+\.md$/;
    var DIGEST_SCHEDULES = ["daily", "weekly"];
    var DEFAULT_SCHEDULE = "daily";
    var DIGEST_FILE_RE = /^digest-([a-z]+)-(\d{8}t\d{6}z)\.md$/;
    var MAX_DIGEST_BYTES = 1024 * 1024;
    function validateDigestName(name) {
      ...
      if (!DIGEST_NAME_RE.test(name)) {
        throw new Error(
          "Invalid digest name: only letters, digits, '.', '_', '-' are allowed, and the name must end with '.md'"
        );
      }
      return name;
    }
    function resolveDigestPath(digestsDir, name) {
      const root = path2.resolve(digestsDir);
      const resolved = path2.resolve(root, String(name));
      const relative = path2.relative(root, resolved);
      if (relative === "" || relative === ".." || relative.startsWith(`..${path2.sep}`) || path2.isAbsolute(relative)) {
    ...
    function createDigestsStore({ digestsDir } = {}) {
      ...
      function ensureDir() {
        fs2.mkdirSync(digestsDir, { recursive: true });
      }
      function write({ title = null, markdown, schedule, generatedAt } = {}) {
        ...
        try {
          fs2.writeFileSync(tmpPath, markdown, "utf8");
          fs2.renameSync(tmpPath, filePath);
        } catch (e) {
          try {
            fs2.unlinkSync(tmpPath);
      ...
      function list() {
        let entries;
        try {
          entries = fs2.readdirSync(digestsDir, { withFileTypes: true });
        } catch (e) {
          if (e.code === "ENOENT") return [];
        ...
        for (const entry of entries) {
          ...
          try {
            const filePath = path2.join(digestsDir, entry.name);
            const stat = fs2.statSync(filePath);
            const content = fs2.readFileSync(filePath, "utf8");
...
var require_digest = __commonJS({
  "src/digest.js"(exports2, module2) {
    var fs2 = require("fs");
    var path2 = require("path");
    var { createDigestsStore } = require_digests_store();
    var TICK_MS = 6e4;
    var HOUR_MS = 36e5;
    var DAY_MS = 864e5;
    ...
    function createDigest({
      ...
      let sending = null;
      let state2 = loadState();
      const digestsStore = typeof digestsDir === "string" && digestsDir.length > 0 ? createDigestsStore({ digestsDir }) : null;
      function loadState() {
...
// src/ntfy-ingest.js
var require_ntfy_ingest = __commonJS({
  "src/ntfy-ingest.js"(exports2, module2) {
    var fs2 = require("fs");
    var path2 = require("path");
    var DEFAULT_INTERVAL_MS = 3e4;
    var RECENT_ID_LIMIT = 500;
    var PRIORITY_SEVERITY = { 1: "info", 2: "info", 3: "info", 4: "warn", 5: "critical" };
    ...
    function normalizeMessage(msg) {
      ...
      return {
        ...
        ts: timeSec !== null ? timeSec * 1e3 : Date.now(),
        node: null,
        task: title
      };
    }
    function createNtfyIngest(options = {}) {
      const {
        ...
        stateFile,
        onAlert,
        fetchImpl = (...args2) => globalThis.fetch(...args2)
      } = options;
      if (!server2 || typeof server2 !== "string") {
      ...
      const baseUrl = server2.replace(/\/+$/, "");
      let lastSeenMs = 0;
      let recentIds = [];
      let recentIdSet = /* @__PURE__ */ new Set();
      let running = false;
      let lastPollAt = null;
      let lastError = null;
      let pollTimer = null;
      function loadState() {
        try {
          ...
          recentIds = Array.isArray(data.recentIds) ? data.recentIds.filter((id) => typeof id === "string").slice(-RECENT_ID_LIMIT) : [];
          recentIdSet = new Set(recentIds);
        } catch (e) {
          lastSeenMs = 0;
          recentIds = [];
          recentIdSet = /* @__PURE__ */ new Set();
        }
      }
      function saveState() {
        try {
      ...
      function parseMessages(text) {
        ...
        for (const line of text.split("\n")) {
          ...
          if (msg && msg.event === "message" && msg.id !== void 0) {
            out.push(msg);
          }
        }
        return out;
      ...
      async function _pollOnce() {
        lastPollAt = Date.now();
        try {
          const sinceSec = lastSeenMs > 0 ? Math.floor(lastSeenMs / 1e3) : 0;
          const url = `${baseUrl}/${encodeURIComponent(topic)}/json?poll=1&since=${sinceSec}`;
...
// src/cron-log.js
var require_cron_log = __commonJS({
  "src/cron-log.js"(exports2, module2) {
    var fs2 = require("fs");
    var path2 = require("path");
    var { execFile: execFile2 } = require("child_process");
    var DEFAULT_INTERVAL_MS = 12e4;
    var DEFAULT_CONTAINER = "openclaw";
    var DEFAULT_RUNS_PER_JOB = 5;
    var EXEC_TIMEOUT_MS = 2e4;
    var EXEC_MAX_BUFFER = 16 * 1024 * 1024;
    var KNOWN_RUNS_LIMIT = 5e3;
    function safeEnv() {
      return {
        PATH: process.env.PATH,
        HOME: process.env.HOME,
    ...
    function createCronLog(options = {}) {
      ...
      function persistKnownRuns() {
        ...
        try {
          ...
          fs2.mkdirSync(path2.dirname(stateFile), { recursive: true });
          fs2.writeFileSync(tmp, JSON.stringify({ ids }), "utf8");
          fs2.renameSync(tmp, stateFile);
        } catch (e) {
      ...
      async function _pollOnce() {
        ...
        for (const event of collected) {
          if (knownRuns.has(event.id)) continue;
...
var require_fleet = __commonJS({
  "src/fleet.js"(exports2, module2) {
    ...
    var { getSystemVitals: getSystemVitals2 } = require_vitals();
    ...
    var { createSubscriptionLimitWatcher } = require_subscription_limit_watcher();
    var { createDigest } = require_digest();
    var { createDigestsStore } = require_digests_store();
    var { createNtfyIngest } = require_ntfy_ingest();
    var { createCronLog } = require_cron_log();
    var { setLatestRunSource } = require_cron();
    var { defaultSecrets } = require_secrets();
    ...
    function createFleetRuntime2({ config, broadcast }) {
      ...
      const selfHostname = config.dispatch && config.dispatch.identity || os2.hostname();
      const mesh = createMesh({
        ...
        getSelfVitals: () => getSystemVitals2(),
      ...
      const NOOP_WATCHER = { start() {
      }, stop() {
      }, evaluate() {
      }, getState: () => ({ running: false }) };
      const subscriptionLimits = subscriptionLimitsEnabled ? createSubscriptionLimitWatcher({
        config: subscriptionLimitsCfg,
        getProviderWindows: () => subscriptionWindowsProvider ? subscriptionWindowsProvider() : [],
        onAlert: (event) => {
          fireAlert({
            type: "subscriptionLimit",
      ...
      let digestExtras = {};
      const digests = createDigestsStore({ digestsDir: path2.join(stateDir, "digests") });
      const digest = createDigest({
        config: config.digest,
        stateFile: path2.join(stateDir, "digest.json"),
        digestsDir: path2.join(stateDir, "digests"),
        sources: {
      ...
      const NOOP_POLLER = { start() {
      }, stop() {
      }, getState: () => ({ running: false }) };
      const ntfyIngest = ntfyIngestEnabled ? createNtfyIngest({
        server: ntfySink.server || "https://ntfy.sh",
        topic: ntfySink.topic,
        intervalMs: ingestCfg.ntfy && ingestCfg.ntfy.intervalMs || 3e4,
        stateFile: path2.join(stateDir, "ntfy-ingest.json"),
      ...
      const cronLog = cronIngestEnabled ? createCronLog({
        intervalMs: ingestCfg.cron && ingestCfg.cron.intervalMs || 12e4,
        stateFile: path2.join(stateDir, "cron-runs-seen.json"),
        onRun: (e) => recordAlert({
          source: "cron",
          type: "cron",
    ...
    function createFleetRoutes2({
      ...
      async function handleKanban(req, res, method, segments, query) {
        ...
        if (segments[1] === "clear-done" && segments.length === 2 && method === "POST") {
          const user = guardMutation(req, res);
          if (!user) return true;
          const count = fleet2.kanban.clearDone(user);
          recordAudit2(user, "task.clear_done", null, { count });
      ...
      async function handleDigests(req, res, method, segments) {
        if (segments.length === 1 && method === "GET") {
          json(res, 200, { digests: fleet2.digests.list() });
          return true;
        }
        if (segments.length !== 2) return false;
        const name = segments[1];
        if (method === "GET") {
          json(res, 200, fleet2.digests.read(name));
          return true;
        }
        return false;
      }
      ...
      async function handleCortex(req, res, method, segments, query) {
        if (segments.length === 1 && method === "GET") {
          ...
          json(res, 200, await fleet2.cortex.getStateRanged(range));
          return true;
        }
      ...
      async function handleAlerts(req, res, method, segments, query) {
        ...
        if (segments.length === 2 && segments[1] === "clear" && method === "POST") {
          const user = guardMutation(req, res);
          if (!user) return true;
          const result = fleet2.alerts.clear();
          recordAudit2(user, "alert.test", null, { op: "clear", cleared: result.cleared });
        ...
        if (segments.length === 2 && method === "DELETE") {
          const user = guardMutation(req, res);
          if (!user) return true;
          const result = fleet2.alerts.dismiss(segments[1]);
          recordAudit2(user, "alert.test", segments[1], { op: "dismiss", dismissed: result.dismissed });
...
// src/dispatch-watchdog.js
var require_dispatch_watchdog = __commonJS({
  "src/dispatch-watchdog.js"(exports2, module2) {
    var dispatchMod = require_dispatch();
    var DEFAULT_CHECK_INTERVAL_MS = 60 * 1e3;
    var DEFAULT_STALE_AFTER_MS = 25 * 60 * 1e3;
    var MIN_STALE_GRACE_MS = 60 * 1e3;
    var DEFAULT_MAX_RETRIES = 2;
    var WATCHDOG_ACTOR = "dispatch-watchdog";
    function openTtlMsFrom(config) {
      const timeoutSec = Number.isInteger(config.timeoutSec) ? config.timeoutSec : 1200;
      return timeoutSec * 1e3 + 15 * 60 * 1e3;
    }
    function createDispatchWatchdog2(options = {}) {
      const {
        kanban,
        config = {},
        dispatchConfig = {},
      ...
      const checkIntervalMs = Number.isInteger(config.checkIntervalMs) ? config.checkIntervalMs : DEFAULT_CHECK_INTERVAL_MS;
      const configuredStaleAfterMs = Number.isInteger(config.staleAfterMs) ? config.staleAfterMs : DEFAULT_STALE_AFTER_MS;
      const timeoutSec = Number.isInteger(dispatchConfig.timeoutSec) ? dispatchConfig.timeoutSec : 1200;
      const minStaleAfterMs = timeoutSec * 1e3 + MIN_STALE_GRACE_MS;
      ...
      function emitReclaimAlert({ taskId, agent, retries, willRetry }) {
        if (typeof fireAlert !== "function") return;
        const message = `Dispatch lock reclaimed for task ${taskId} (agent ${agent}) \u2014 ` + (willRetry ? `silent past threshold, re-dispatching (retry ${retries}/${maxRetries})` : `silent past threshold, retry cap (${maxRetries}) reached \u2192 marked failed`);
        try {
          Promise.resolve(
            fireAlert({
            ...
            (e) => console.error("[DispatchWatchdog] dispatchReclaimed alert failed:", e.message)
          );
        } catch (e) {
      ...
      function reclaim(task, open, nowMs) {
        ...
        const prior = state2.get(taskId) || { retries: 0 };
        ...
        try {
          kanban.updateAttempt(taskId, open.index, {
            ...
            result: "failure",
            note: `${dispatchMod.DISPATCH_NOTE} \xB7 reclaimed: no activity past ${Math.round(
              staleAfterMs / 1e3
            )}s (watchdog)`
          });
        } catch (e) {
        ...
        try {
          kanban.addComment(taskId, {
            ...
            text: willRetry ? `[Watchdog] Reclaimed stuck dispatch (agent ${agent}); re-dispatching (retry ${retries}/${maxRetries}).` : `[Watchdog] Reclaimed stuck dispatch (agent ${agent}); retry cap (${maxRetries}) reached \u2014 needs attention.`
          });
        } catch (e) {
        ...
        state2.set(taskId, { retries, lastReclaimedStartedAt: open.attempt.started_at });
    ...
    module2.exports = { createDispatchWatchdog: createDispatchWatchdog2 };
  }
});
...
var require_settings = __commonJS({
  "src/settings.js"(exports2, module2) {
    ...
    var SSRF_DENIED_HOSTNAMES = /* @__PURE__ */ new Set([
      "localhost",
    ...
    function assertSafeUrlHost(parsed, label) {
      ...
      if (SSRF_DENIED_HOSTNAMES.has(host) || host.endsWith(".internal") || isDeniedIPv4(host) || isDeniedIPv6(host)) {
        throw badRequest(
...
// src/usage-sources/plan-usage.js
var require_plan_usage = __commonJS({
  "src/usage-sources/plan-usage.js"(exports2, module2) {
    var fs2 = require("fs");
    var os2 = require("os");
    var path2 = require("path");
    var DEFAULT_STATS_PATH = path2.join(
      os2.homedir(),
      ".local",
      "state",
      "openclaw-quota",
    ...
    function normalizeCodexCredits(credits) {
      ...
      return {
        balance: toFiniteOrNull2(credits.balance),
    ...
    function createPlanUsageSource(options = {}) {
      ...
      function readState() {
        const status2 = describe();
        if (!status2.available) return { error: status2.reason };
        let data;
        try {
      ...
      async function getSubscription() {
        ...
        if (state2.error) return { available: false, reason: state2.error };
    ...
    module2.exports = { createPlanUsageSource };
  }
});
...
var require_usage_sources = __commonJS({
  "src/usage-sources/index.js"(exports2, module2) {
    ...
    var { createPlanUsageSource } = require_plan_usage();
    ...
    function createUsageSources2(config = {}) {
      ...
      async function getAll(params = {}) {
        const [claudeCodeData, codexData, nineRouterData, planUsageData, openrouterData] = await Promise.all([
    ...
    function createAgentsRoster2(options = {}) {
      ...
      async function getAssignees() {
        ...
        for (const agent of roster.agents) {
          ...
          const nodes = nodesById.get(agent.id) || /* @__PURE__ */ new Set();
...
var require_bind_host = __commonJS({
  "src/bind-host.js"(exports2, module2) {
    ...
    function assertSecureBindPosture2(authConfig, bindHost, { warn = console.warn } = {}) {
      const errors = [];
      const warnings = [];
      const mode = authConfig && typeof authConfig.mode === "string" ? authConfig.mode : "none";
      const bindsAllInterfaces = resolveBindHost2(bindHost) === null;
      if (mode === "none" && bindsAllInterfaces) {
        errors.push(
          'REFUSING TO START: auth.mode is "none" while the server binds ALL interfaces (server.bindHost is unset/0.0.0.0). This would expose an UNAUTHENTICATED fleet control plane to the entire network. Set auth.mode to "tailscale"/"cloudflare"/"token"/"allowlist", or set server.bindHost to "127.0.0.1" (loopback only).'
        );
      }
      if (mode === "tailscale") {
        ...
        if (!verify) {
          warnings.push(
            'SECURITY WARNING: auth.mode is "tailscale" but auth.tailscale.verifyServeOrigin is false. The tailscale-user-login identity header is then trusted as-is and can be forged by any direct tailnet connection to the bound port. Set auth.tailscale.verifyServeOrigin=true and bind loopback behind Tailscale Serve.'
          );
        }
      }
    ...
    module2.exports = { resolveBindHost: resolveBindHost2, assertSecureBindPosture: assertSecureBindPosture2 };
  }
});

// src/http-security.js
var require_http_security = __commonJS({
  "src/http-security.js"(exports2, module2) {
    var UNSAFE_METHODS = /* @__PURE__ */ new Set(["POST", "PUT", "PATCH", "DELETE"]);
    var CSRF_HEADER = "x-ofc-csrf";
    function securityHeaders2() {
      return {
    ...
    function checkCrossOrigin2(req) {
    ...
    module2.exports = {
      securityHeaders: securityHeaders2,
      checkCrossOrigin: checkCrossOrigin2,
      originKey,
      UNSAFE_METHODS,
      CSRF_HEADER
    };
...
var { createDispatchWatchdog } = require_dispatch_watchdog();
...
var { createFlightRecorderRoutes, isFlightRecorderRoute } = require_run_archive_routes();
var { createSessionControl } = require_session_control();
var { createRateLimiter } = require_rate_limit();
var { resolveBindHost, assertSecureBindPosture } = require_bind_host();
var { createTailscaleWhois, verifyServeLogin } = require_auth();
var { securityHeaders, checkCrossOrigin } = require_http_security();
...
var dispatchWatchdogCfg = CONFIG.fleet.dispatchWatchdog || {};
var dispatchWatchdogEnabled = dispatchWatchdogCfg.enabled !== false;
var NOOP_DISPATCH_WATCHDOG = {
  check: () => [],
  start() {
  },
  stop() {
...
var dispatchWatchdog = dispatchWatchdogEnabled ? createDispatchWatchdog({
  kanban: fleet.kanban,
...
async function postBoardThreadParent({ title, question }) {
  ...
  const text = `\u{1F9D1}\u200D\u2696\uFE0F ${chiefMention}*Board:* ${title}
> ${question}
_Advisors are replying in this thread\u2026_`;
  ...
  try {
    ...
    return parsed && parsed.messageId || parsed && parsed.payload && parsed.payload.result && parsed.payload.result.receipt && parsed.payload.result.receipt.primaryPlatformMessageId || null;
  } catch (e) {
...
var planUsageStats = CONFIG.fleet.usage.planUsageStats || CONFIG.fleet.cortex.planUsageStats;
...
fleet.setSubscriptionWindowsProvider(async () => {
  ...
  return windows.filter(([, , w]) => w && Number.isFinite(w.utilizationPct)).map(([provider, window, w]) => ({
    provider,
    window,
...
var posture = assertSecureBindPosture(AUTH_CONFIG, CONFIG.server.bindHost);
if (posture.fatal) {
