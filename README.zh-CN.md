...
| 📋 **看板（Kanban）**                                 | 蜂群任务看板：`inbox → assigned → inprogress → review → done \| failed`。看板文件允许代理直接编辑；每次读取都经过安全存储层，自动隔离损坏的 JSON 并恢复备份。看门狗会标记停滞的进行中任务。**完整键盘无障碍**（v1.6）：`Tab` 在卡片间移动，`Enter`/`Space` 打开详情抽屉（焦点锁定，`Esc` 关闭），`M` 切换移动模式，随后 `←`/`→` 切换列、`↑`/`↓` 在列内调整位置，`Enter` 确认、`Esc` 取消并恢复原位——每次移动都通过 `aria-live` 区域播报。                                                                                                         |
...
| 🌐 **联邦（Federation）**                             | 舰队之舰队：注册其他 Open Fleet Control 仪表盘（仅限 HTTPS，可选 Bearer Token——仅存储于服务器端、响应中始终脱敏），在同一块屏幕上观察它们的紧凑舰队摘要。远端**默认只读**；v1.6 新增**可选写操作**：按远端开启 `allowWrites` 开关（卡片上的 WRITES 徽标）后，卡片会展示远端的待审经验教训与进化闸门。所有写操作都经由服务器端代理，仅执行**硬编码的动作白名单**（`lesson.approve`、`lesson.reject`、`gate.set`、`task.move`）并进行严格参数校验，同时**转发你的操作者身份**（`Tailscale-User-Login`），使该操作在两侧仪表盘都以你的名义记入审计。 |
...
| 🌍 **面板全量 i18n**                                  | 整个面板——HTML 外壳**以及**所有由 JS 运行时生成的字符串（提示、确认对话框、空/错误状态、aria-label、读屏播报、工具提示）——均已键化，提供 **English 与简体中文**（`public/locales/en.json` / `zh-CN.json`）。语言依次取自 `?lang=`、`localStorage`（`occ.locale`）、`navigator.language`，可在页头实时切换；缺失键始终回退到英文，绝不渲染原始键路径。`scripts/checks/i18n-coverage.mjs` 在 CI 中保证代码与语言包同步。                                                                                                                            |
...
| `/api/fleet/federation/remotes/:id`                    | PATCH / DELETE     | 切换远端的 `allowWrites` / 移除远端                                                           |
| `/api/fleet/federation/remotes/:id/actions`            | POST               | 白名单写代理（`lesson.approve` · `lesson.reject` · `gate.set` · `task.move`），转发操作者身份 |
...
- **联邦远端节点下钻** —— 从联邦卡片点进远端的网格/看板详情，而不止于摘要瓦片。
- **图谱边抽取调度** —— 在 Cortex 面板中管理（并可视化）gbrain 的周期性边抽取任务。
- **告警规则 UI** —— 在仪表盘中直接编辑告警规则与接收端，而非修改 `dashboard.json`。
