...
| Cortex        | `src/cortex.js`, `src/cortex-gbrain.js`, `src/cortex-gauges.js` | Facade over two adapters: gbrain knowledge graph as the single-store memory engine (read-only via CLI — Cortex lists gbrain pages, never opens the PGLite DB, and all writes happen via the nightly gbrain sync), and compression gauges (headroom / lean-ctx / lcm token-savings telemetry). All lazy, all degrade to `{ error }` instead of throwing.                                                                            |
...
│  └────────┘ └──────────┘ │ └────────────┘                    │
│  ┌──────────────────────┴────────────────────────────┐       │
│  │ cortex facade → gbrain CLI / gauge files           │       │
│  └───────────────────────────────────────────────────┘       │
└──────────┬───────────────────────────────────────────────────┘
           ▼
